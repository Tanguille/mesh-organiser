...
pub async fn download_file(url: &str) -> Result<DownloadResult, ServiceError> {
    ...
    let redirect_url_filename = match response.url().path_segments() {
        Some(mut segments) => {
            String::from(decode(segments.next_back().unwrap_or("model.stl")).unwrap())
        }
    ...
    let mut file_name = url.split('/').next_back().unwrap_or("model.stl");
    let mut tmp: String;
    ...
    } else if let Some(filename) = get_content_disposition_filename(&response) {
        tmp = filename;
        file_name = &tmp;
    }
