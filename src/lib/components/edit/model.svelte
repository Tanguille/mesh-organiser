<script lang="ts">
  ...
  let load3dPreview = $derived(
    loadModelAutomatically($state.snapshot(configuration), model),
  );
...
{:else}
  <Card class={props.class}>
    <CardHeader class="relative flex flex-col gap-2 space-y-0">
      <div class="flex h-9 flex-row gap-2">
        <Badge
          class="my-auto h-fit text-sm {fileTypeToColor(model.blob.filetype)}"
          >{fileTypeToDisplayName(model.blob.filetype)}</Badge
        >
        ...
        <Toggle
          size="sm"
          class={isModelPreviewable(model) && load3dPreview ? "" : "hidden"}
          bind:pressed={doNotAutoRotateModel}
        >
          <Pin />
        </Toggle>
        ...
        {#if !configurationMeta.applicationReadOnly}
          {#if shareApi}
            <Button
              ...
              onclick={() => createShare([model], shareApi)}
              variant="ghost"
            >
              <Share2 />
            </Button>
          ...
          <DropdownMenu.Root>
            <DropdownMenu.Trigger>
              {#snippet child({ props })}
                <Button {...props} class="widthhack h-full" variant="ghost">
                  <ListCheck />
                </Button>
              {/snippet}
            </DropdownMenu.Trigger>
        ...
        <Toggle
          size="sm"
          class={isModelPreviewable(model) ? "" : "hidden"}
          bind:pressed={load3dPreview}
        >
          <Box />
        </Toggle>
          ...
          <DropdownMenu.Root>
            ...
            <DropdownMenu.Content side="right" align="start">
              ...
              {#if model.blob.filetype == FileType.THREEMF && threemfApi}
                <DropdownMenu.Item
                  onclick={async () =>
                    extractThreemfModels(model, threemfApi, groupApi)}
                >
        ...
        {:else if !configurationMeta.applicationReadOnly}
          <Button
            size="sm"
            variant="ghost"
            class="widthhack aspect-square h-full"
      ...
      <div class="aspect-square h-full max-h-[512px]">
        {#if load3dPreview}
          <ThreeCanvas
            {model}
            class="h-full"
            autoRotate={!doNotAutoRotateModel}
          />
        {:else}
    ...
    <CardContent class="pt-4 text-sm">
      <div class="mb-4 grid grid-cols-2 gap-4">
        {#if localApi}
          <AsyncButton class="grow" onclick={onOpenInFolder}
            ><FolderOpen /> Open in folder</AsyncButton
          >
        {:else if downloadApi}
          <AsyncButton class="grow" onclick={onDownloadModel}
      ...
      <div class="mt-4 flex flex-col space-y-1.5">
        <div class="grid grid-cols-2 text-sm">
          ...
          <div class="space-y-1 text-right">
            ...
            {#if group}
              <a
                href={resolve("/group/" + group.id)}
                class="block overflow-x-hidden text-ellipsis whitespace-nowrap text-primary hover:underline"
                >{group.name}</a
              >
            {:else}
              <div>None</div>
  ...
  <div class="grid w-full items-center gap-2">
    ...
    {#if model.labels.length > 0}
      <div class="mt-2 flex flex-row flex-wrap gap-2">
        {#each model.labels as label (label.id)}
