...
{#if !deleted}
  <Card class={props.class}>
    <CardHeader class="relative">
      <div
        class="{props.settingsVertical
          ? 'mr-10 grid grid-cols-1'
          : 'flex flex-row'} gap-2"
      >
        <CardTitle>Group '{tracked_group.meta.name}'</CardTitle>
        <p class="my-auto ml-2 text-xs font-thin">
          Created {tracked_group.meta.created.toLocaleDateString()}
        </p>
        {#if !!resource}
          <p class="my-auto ml-2 text-xs font-thin">
            Part of project '{resource.name}'
          </p>
        {/if}
      </div>
...
{#snippet ViewContent()}
  <div class="flex flex-row gap-4">
    <LinkButton
      class="grow"
      {link}
      visible={!(link_disabled || link === null)}
      withFallback={true}
    />
    {#if resourceFolderApi}
      <Button class="grow" disabled={!resource} onclick={openResourceInFolder}
        ><NotebookText /> Open project</Button
      >
    {/if}
  </div>
{/snippet}
  ...
  >
    ...
    <div class="flex flex-col space-y-1.5">
      ...
      <div class="flex flex-row gap-2">
        ...
        />
        {#if resource}
          <Button class="h-full" onclick={openResourceInFolder}
            ><NotebookText /> Open project</Button
          >
        {:else if resourceFolderApi}
          <Button class="h-full" onclick={onNewResource}
            ><NotebookPen /> Create project</Button
          >
        {/if}
      </div>
    </div>
