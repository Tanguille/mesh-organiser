<script lang="ts">
  ...
  let availableLabels = $derived(
    sidebarState.labels
      .map((l) => l.meta)
      .filter((l) => l.id !== tracked_label.meta.id),
  );
...
<Card class={props.class}>
  <CardHeader class="relative">
    <CardTitle class="mr-10"
      >{!thisLabelOnly && tracked_label.children.length > 0 ? "Grouped" : ""} Label
      '{tracked_label.meta.name}'</CardTitle
    >
    <div class="absolute top-5 right-0 mr-8 flex gap-5">
      <EditListPopover
        title="Edit keywords"
        description="When an imported model's name contains any previously defined keywords, the associated label will automatically be added to the model."
        bind:value={keywords}
        onEdit={updateKeywords}
      >
        <Button size="sm"
  ...
  <CardContent class="text-sm">
    <div class="grid grid-cols-3 gap-4">
      ...
      <div class="flex flex-col space-y-1.5">
        ...
        <LabelSelect
          placeholder="Add sub-labels"
          {availableLabels}
          onchange={onUpdateLabel}
          bind:value={tracked_label.children}
        />
      </div>
    </div>
  </CardContent>
