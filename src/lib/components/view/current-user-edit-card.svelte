<script lang="ts">
  ...
  const diskUsageInfoApi =
    getContainer().optional<IDiskUsageInfoApi>(IDiskUsageInfoApi);
  ...
  <CardContent class="flex flex-col gap-5 text-sm">
    ...
    {#if !isDesktop}
      ...
      <div class="flex flex-col gap-3">
        ...
        <Input id="password2" type="password" bind:value={password2} />

        <AsyncButton class="mt-2" onclick={changePassword}
      ...
      {#if userTokenApi}
        ...
        <Button
          class="w-full"
          variant="destructive"
          onclick={resetDesktopInstances}>Reset linked desktop instances</Button
        >
      {/if}
    {:else if currentUser.syncToken}
      <Button class="w-full" variant="destructive" onclick={clearSyncState}
        >Clear sync state</Button
      >
    {/if}
      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="grid grid-cols-2 text-sm">
          ...
          <div class="space-y-1 text-right">
            ...
            <div>
              {Number(
                ((diskUsage.size_uncompressed - diskUsage.size_compressed) /
                  diskUsage.size_uncompressed) *
                  100,
              ).toFixed(1)}%
            </div>
          </div>
