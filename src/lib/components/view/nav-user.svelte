<script lang="ts">
  ...
  let filteredUsers = $derived(
    availableUsers.filter((x) => x.id !== currentUser?.id),
  );
      ...
      >
        <DropdownMenu.Trigger>
          {#snippet child({ props })}
            ...
            >
              <div
                class="rounded-md bg-sidebar-accent p-1 text-sidebar-accent-foreground"
              >
        ...
        <DropdownMenu.Content>
          ...
          {:else if switchUserApi}
            ...
            <DropdownMenu.Label class="font-normal"
              >No other users available. {currentUser!.permissions.admin
                ? "See settings to create new users"
                : ""}</DropdownMenu.Label
            >
          {/if}
          ...
          {#if userTokenApi}
            <DropdownMenu.Item
              onclick={async () =>
                await userTokenApi.openMeshOrganiserDesktopWithToken()}
            >
              <Link />
          ...
          <DropdownMenu.Item onclick={() => goto(resolve("/about"))}>
            <CircleHelp />
            About
          </DropdownMenu.Item>
          {#if diskUsage}
