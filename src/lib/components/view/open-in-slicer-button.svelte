...
{#if sidebarApi}
  <AsyncButton
    class={componentProps.class?.toString() ?? ""}
    onclick={onOpenInSlicer}><Slice /> Open in slicer</AsyncButton
  >
{:else}
  <DropdownMenu.Root>
    <DropdownMenu.Trigger>
      {#snippet child({ props: triggerProps })}
        <Button {...triggerProps} class={componentProps.class}
    ...
    <DropdownMenu.Content align="start">
      {#each slicers as slicer (slicer.slicer)}
        <DropdownMenu.Item onclick={() => onOpenInSpecificSlicer(slicer)}
          >Open in {slicer.slicer}</DropdownMenu.Item
        >
      {/each}
