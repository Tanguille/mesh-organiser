<script lang="ts">
  ...
  async function linkLocalAccount() {
    ...
    try {
      ...
      await userSyncApi.setSyncState(
        props.data.baseUrl,
  ...
  async function createOnlineAccount() {
    ...
    try {
      let newUser = await userAdminApi.addUser(
        props.data.userName.trim(),
        Math.round(Math.random() * 10000000) + "@noemail.com",
        "none",
      );
  ...
  onMount(async () => {
    if (userAdminApi) {
      users = (await userAdminApi.getAllUsers()).filter(
        (u) => !(u.permissions.onlineAccount || !!u.syncToken),
      );
    }
  ...
  <Card class="w-[800px]">
    <CardHeader>
      ...
      <p>
        ...
        importing models in all major slicers (including a custom slicer),
        exporting models into a folder, accepting 'Open in' links from model
        websites, (recursive) folder imports, deleting models after importing,
        and the internal browser to browse model websites.
      </p>
    </CardHeader>
    <CardContent class="mt-10 flex flex-col gap-4">
      <div class="grid grid-cols-[1fr_auto_1fr] gap-10">
        <div class="flex w-full flex-col items-center justify-start gap-2">
          <FolderSync />
          <p>Local Sync</p>
          <p>
            Sync all your models between your local PC and your online account.
            This speeds up loading times and gives access to your models while
            offline, at the cost of local storage space.
          </p>
        </div>
        ...
        <div class="flex w-full flex-col items-center justify-start gap-2">
          ...
          <p>Online Only</p>
          <p>
            Access online accounts inside the desktop application. These
            accounts will be inaccessible while offline.
          </p>
        </div>
      </div>
        ...
        <div>
          ...
          <Select.Root
            ...
            bind:value={
              ...
              (val) =>
                (currentUser =
                  users.find((u) => u.id === parseInt(val)) || null)
            }
          >
            <Select.Trigger>
            ...
            <Select.Content>
              <Select.Group>
                ...
                {#each users as user (user.id)}
                  <Select.Item value={user.id.toString()} label={user.username}>
                    {user.username}
                  </Select.Item>
                {/each}
              </Select.Group>
