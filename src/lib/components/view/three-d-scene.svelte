<script lang="ts">
  ...
  const props: { geometry: BufferGeometry | null; autoRotate?: boolean } =
    $props();
    ...
    surfaceColor: {
      value: (() => {
        ...
        const linearColor = new Color(
          $state.snapshot(configuration.thumbnail_color),
        ).toArray();
...
{#if props.geometry}
  ...
  >
    <OrbitControls enableDamping={true} autoRotate={props.autoRotate ?? true}
    ></OrbitControls>
  </T.PerspectiveCamera>
