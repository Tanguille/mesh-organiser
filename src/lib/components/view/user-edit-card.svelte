<script lang="ts">
  ...
  async function resetPassword(user: User): Promise<void> {
    ...
    toast.success(
      `Password for user '${user.email}' reset to '${newPassword}'. The password has also been copied to your clipboard.`,
      { duration: 10000 },
    );
  ...
  async function addUser(): Promise<void> {
    ...
    let createdUser = await userAdminApi.addUser(
      newUser.username,
      newUser.email,
      password,
    );
    ...
    if (!isDesktop) {
      try {
        ...
        toast.success(
          `User '${createdUser.username}' created. Login details have been copied to your clipboard.`,
          { duration: 10000 },
        );
      } catch (e) {
        console.error("Failed to copy password to clipboard", e);
  ...
  <CardContent class="flex flex-col gap-2">
    {#each users.filter((x) => x.id > 1 && x.id != currentUser.id) as user (user.id)}
      <div class="mr-1 flex flex-row gap-2">
        <p class="grow truncate text-sm capitalize">{user.username}</p>
        {#if !isDesktop}
          <Popover.Root>
            <Popover.Trigger
              class={buttonVariants({ variant: "ghost", size: "mi" })}
            >
              <KeyRound />
            </Popover.Trigger>
            <Popover.Content class="flex w-80 flex-col gap-10">
              <h1 class="text-center font-bold">Reset password</h1>
              <Button variant="destructive" onclick={() => resetPassword(user)}
                >Reset password for '{user.username}'</Button
              >
            </Popover.Content>
          </Popover.Root>
        {/if}
            ...
            <div class="grid gap-4">
              ...
              <div class="grid gap-2">
                ...
                {#if !isDesktop}
                  ...
                  <CheckboxWithLabel
                    bind:value={user.permissions.admin}
                    label="Admin user"
                  />
                {/if}
              </div>
        ...
        <Popover.Root>
          ...
          <Popover.Content class="flex w-80 flex-col gap-10">
            ...
            <Button variant="destructive" onclick={() => deleteUser(user)}
              >Delete account '{user.username}'</Button
            >
          </Popover.Content>
        </Popover.Root>
      </div>
    ...
    >
      ...
      <Popover.Content class="w-80">
        <div class="grid gap-4">
          <div class="space-y-2">
            <h4 class="leading-none font-medium">Create user</h4>
          </div>
          <div class="grid gap-2">
            <div class="grid grid-cols-3 items-center gap-4">
              <Label for="username">Username</Label>
