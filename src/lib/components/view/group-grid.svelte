<script lang="ts">
  ...
  function onDelete() {
    ...
    for (const group of affectedGroups) {
      let groupIndex = loadedGroups.findIndex(
        (g) => g.meta.id === group.meta.meta.id,
      );
      if (group.fullGroup) {
        loadedGroups.splice(groupIndex, 1);
      } else {
        loadedGroups[groupIndex].models = loadedGroups[
          groupIndex
        ].models.filter((m) => !set.has(m.id));
      }
    }
  ...
  function onGroupDelete(deletedGroup: Group) {
    let groupIndex = loadedGroups.findIndex(
      (g) => g.meta.id === deletedGroup.meta.id,
    );
    if (groupIndex !== -1) {
      loadedGroups.splice(groupIndex, 1);
      deletedGroup.models.reverse().forEach((m) => {
        let newGroup: Group = {
          meta: createGroupMetaInstance(
            m.id * -1,
            m.name,
            m.added.toISOString(),
  ...
  {#if showRightSide}
    <div
      class="{isMobile.current
        ? 'w-full'
        : 'w-[400px] min-w-[400px]'} relative mx-4 my-2 hide-scrollbar flex flex-col gap-4 overflow-y-auto"
    >
      {#if isMobile.current}
        <Button
          onclick={() => {
      ...
      {:else if selected.length === 1 && selected[0].meta.id >= 0}
        <EditGroup
          ...
          onDelete={() => onGroupDelete(selected[0])}
        />
          ...
          {#each loadedGroups as group (group.meta.id)}
            ...
            <div class="grid w-full grid-cols-[auto_1fr] items-center gap-2">
              ...
              >
                <GroupTinyList
                  {group}
                  class="{size} pointer-events-none select-none {isSelected
                    ? 'border-primary'
                    : ''}"
                />
              </div>
