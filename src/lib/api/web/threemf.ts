...
export class WebThreemfApi implements IThreemfApi {
  ...
  async getThreemfMetadata(modelId: Model): Promise<ThreemfMetadata | null> {
    try {
      return await this.requestApi.request<ThreemfMetadata>(
        `/models/${modelId.id}/3mf_metadata`,
        HttpMethod.GET,
      );
    } catch {
      return null;
  ...
  async extractThreemfModels(modelId: Model): Promise<GroupMeta> {
    const groupMeta = await this.requestApi.request<RawGroupMeta>(
      `/models/${modelId.id}/3mf_extract`,
      HttpMethod.POST,
    );
