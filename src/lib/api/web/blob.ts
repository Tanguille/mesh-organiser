...
export class WebBlobApi implements IBlobApi {
  ...
  async getBlobDownloadUrl(blob: Blob): Promise<string> {
    return (
      this.hostUrl +
      `/api/v1/blobs/${blob.sha256}/download?user_hash=${this.user.syncUrl}&user_id=${this.user.id}`
    );
  }

  async getBlobsDownloadUrl(blobs: Blob[]): Promise<string> {
    const data = blobs.map((b) => b.sha256);
    const zipDir = await this.requestApi.request<string>(
      "/blobs/download",
      HttpMethod.POST,
      data,
    );
  ...
  async getBlobThumbnailUrl(blob: Blob): Promise<string> {
    return blobThumbnailUrl(this.hostUrl, blob.sha256);
  }
}
