...
async function stepUploadToRemote(
	...
	for (const resource of toUpload) {
		...
		let relatedRemoteGroups = remoteGroups.filter((x) =>
...
async function stepSyncToRemote(
	...
	for (const resourceSet of toSync) {
		let remoteResource = isServerToLocal
			? resourceSet.local
			: resourceSet.server;
		let localResource = isServerToLocal
			? resourceSet.server
			: resourceSet.local;

		let localResourceGroups =
			await localApi.getGroupsForResource(localResource);
		let remoteResourceGroups =
		...
		let toRemoveGroups = remoteResourceGroups.filter(
			(x) =>
...
export async function syncResources(
	...
	let syncState = computeDifferences(
		localResources,
		serverResources,
		lastSynced,
	);
