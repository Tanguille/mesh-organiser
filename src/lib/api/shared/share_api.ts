...
export function createShareInstance(
  id: string,
  createdAt: string,
  shareName: string,
  userName: string,
...
export async function createShare(
  models: Model[],
  shareApi: IShareApi | null,
): Promise<void> {
  if (!shareApi) {
    return;
  }

  const existingShares = await shareApi.getShares();
  const modelIds = models.map((m) => m.id);
  const sameShare = existingShares.find(
    (share) =>
      share.modelIds.length === modelIds.length &&
      share.modelIds.every((id) => modelIds.includes(id)),
  );
  ...
  const link = await shareApi.getShareLink(share);
  await navigator.clipboard.writeText(link);

  toast.success(
    "Share created successfully. Link has been copied to your clipboard.",
  );
}
