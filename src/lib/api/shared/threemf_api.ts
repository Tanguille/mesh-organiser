...
export async function extractThreemfModels(
  model: Model,
  threemfApi: IThreemfApi | null,
  groupApi: IGroupApi | null,
): Promise<void> {
  if (!threemfApi || !groupApi) {
    return;
  }

  const promise = threemfApi.extractThreemfModels(model);
