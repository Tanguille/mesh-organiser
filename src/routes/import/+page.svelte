    ...
    <div class="flex w-full flex-col gap-1">
      <div class="mt-4 flex flex-row justify-center gap-5">
        <Button onclick={resetImportState}
          ><Undo2 /> Import another model</Button
        >
        <div class="my-auto">
      ...
      {#if importedGroups.length === 1 && importedGroups[0].meta.id >= 0}
        <div class="overflow-hidden">
          <GroupPage
            ...
            group={importedGroups[0].meta}
            onGroupDelete={() => (importedGroups[0].meta.id = -1)}
            onAllModelsDelete={resetImportState}
          />
        </div>
      {:else if importedModels.length > 0}
        <div class="w-full grow overflow-hidden">
          <ModelGrid
            modelStream={new PredefinedModelStreamManager(importedModels)}
            default_show_multiselect_all={true}
            initialEditMode={true}
            onEmpty={resetImportState}
          />
        </div>
      {:else}
        ...
        <Card>
          ...
          <CardContent class="flex flex-col gap-4">
            <div class="grid grid-cols-2 gap-4">
              <Button
                class="grow"
                onclick={handleTauriOpenFile}
                disabled={dialog_open}><File /> Import File</Button
              >
              <Button
                class="grow"
                onclick={handleTauriOpenFolder}
            ...
            <CheckboxWithLabel
              label="Import folder recursively"
              bind:value={globalImportSettings.recursive}
            />
            <CheckboxWithLabel
              label="Delete files after import"
              bind:value={globalImportSettings.delete_after_import}
              disabled={globalImportSettings.import_as_path}
            />
          </CardContent>
        </Card>
      {/if}
        ...
        <Card>
          ...
          <CardContent class="flex flex-col gap-4">
            <Button
              ...
              disabled={dialog_open}><File /> Import Files</Button
            >
          </CardContent>
        </Card>
      {/if}
      ...
      {#if internalBrowserApi}
        <Card>
          <CardHeader>
            ...
            <CardDescription
              >Browse external repositories in a new window.{#if isDesktop}<br
                />Downloads are redirected to this application.{/if}</CardDescription
            >
          </CardHeader>
