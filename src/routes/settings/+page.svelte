<script lang="ts">
  ...
  const tauriImportApi =
    getContainer().optional<ITauriImportApi>(ITauriImportApi);
  let sections = $state<SettingSection[]>(
    settingsApi
      ? settingsApi.availableSections()
      : Object.values(SettingSection).map((x) => x as SettingSection),
  );
  ...
  >
    {#if sections.includes(SettingSection.ThumbnailGeneration)}
      <Card>
        ...
        <CardContent class="flex flex-col gap-5 text-sm">
          {#if thumbnail_regen_button_enabled && thumbnailApi}
            <div class="mb-4 grid grid-cols-2 gap-4">
              <Button onclick={() => replaceAllThumbnails(true)}
                >Regenerate all thumbnails</Button
              >
              <Button onclick={() => replaceAllThumbnails(false)}>
                Generate missing thumbnails
              </Button>
            </div>
          {:else if thumbnailApi}
            <Label class="mx-auto p-2"
              >Progress: {(
                (importState.finished_thumbnails_count /
                  importState.model_count) *
                100
              ).toFixed(1)}%</Label
            >
          {/if}
          ...
          <CheckboxWithLabel
            bind:value={configuration.prefer_gcode_thumbnail}
            label="Prefer gcode thumbnail over gcode model"
          />
          <CheckboxWithLabel
            bind:value={configuration.fallback_3mf_thumbnail}
            label="Use fallback thumbnail for 3mf files"
          />
          {#if configuration.fallback_3mf_thumbnail}
            <CheckboxWithLabel
              class="ml-8"
              bind:value={configuration.prefer_3mf_thumbnail}
              label="Prefer 3mf thumbnail over 3mf model"
            />
          {/if}
        ...
        <CardFooter class="flex flex-col gap-2">
          ...
          <div>
            Note: Images may not update in the application until the application
            is restarted.
          </div>
          <div>
            Note: X rotation moves the camera side to side (with +X moving
            right), Y rotation moves the camera up and down (with +Y moving up),
            Z rotation spins the camera (with +Z spinning clockwise).
          </div>
        </CardFooter>
      </Card>
    {/if}
    ...
    {#if sections.includes(SettingSection.ImportExport)}
      <Card>
        ...
        <CardContent class="flex flex-col gap-5 text-sm">
          <CheckboxWithLabel
            bind:value={configuration.default_enabled_recursive_import}
            label="Check 'Import folder recursively' by default"
          />
          <CheckboxWithLabel
            bind:value={configuration.default_enabled_delete_after_import}
            label="Check 'Delete files after import' by default"
          />

          <CheckboxWithLabel
            bind:value={configuration.export_metadata}
            label="Export metadata to .json when opening in folder"
          />
          <CheckboxWithLabel
            bind:value={
          ...
          <div
            class="flex flex-col space-y-1.5 rounded-md border border-destructive p-4"
          >
            <CheckboxWithLabel
              bind:value={
                () => configuration.default_enabled_import_as_path,
                (val) => {
                  configuration.default_enabled_import_as_path = val;
                  globalImportSettings.import_as_path = val;
                }
              }
              label="Reuse files on disk, do not import into internal registry"
            />
            <p>
              Mesh Organiser makes use of an internal file registry to manage
              imported models. This way, if the original files were deleted or
              moved, the models within Mesh Organiser would stay valid. The
              models also get compressed this way. Enabling the toggle above
              instead uses the location on disk of the imported files, instead
              of the internal registry. Moving or deleting imported model files
              will cause issues when this feature is enabled!
            </p>
          </div>
          ...
          <div class="flex flex-col space-y-1.5">
            ...
            <div class="flex flex-row gap-2">
              ...
              <Button onclick={() => (configuration.data_path = app_data_dir)}
                >Default</Button
              >
            </div>
            <div>
    ...
    {#if sections.includes(SettingSection.DeepLink)}
      <Card>
        ...
        <CardContent class="flex flex-col gap-5 text-sm">
          <CheckboxWithLabel
            bind:value={configuration.prusa_deep_link}
            label="Bind 'Open in PrusaSlicer' links"
          />
          <CheckboxWithLabel
            bind:value={configuration.cura_deep_link}
            label="Bind 'Open in Cura' links"
          />
          <CheckboxWithLabel
            bind:value={configuration.bambu_deep_link}
            label="Bind 'Open in Bambu Studio' links"
          />
          <CheckboxWithLabel
            bind:value={configuration.orca_deep_link}
            label="Bind 'Open in OrcaSlicer' links"
          />
          <CheckboxWithLabel
            bind:value={configuration.elegoo_deep_link}
            label="Bind 'Open in Elegoo Slicer' links"
          />
          <div>
            Note: Binding the same link as your current slicer may cause opening
            links of that type to become inconsistent.
          </div>
        </CardContent>
      </Card>
    {/if}
    ...
    {#if sections.includes(SettingSection.CustomSlicer)}
      <Card>
        ...
        <CardContent class="flex flex-col gap-5 text-sm">
          ...
          <div>
            Note: On all platforms, this will run the chosen file as executable,
            with all opened models as arguments.
          </div>
        </CardContent>
      </Card>
    {/if}
      ...
      <Card>
        ...
        <CardContent class="flex flex-col gap-5 text-sm">
          {#if sections.includes(SettingSection.Behaviour)}
            <CheckboxWithLabel
              bind:value={configuration.open_slicer_on_remote_model_import}
              label="Open slicer after importing from website"
            />
            <CheckboxWithLabel
              bind:value={configuration.focus_after_link_import}
              label="Focus window after importing from website"
            />
            <CheckboxWithLabel
              bind:value={configuration.open_links_in_external_browser}
              label="Open links in external browser"
            />
          {/if}
          <CheckboxWithLabel
            bind:value={configuration.label_exported_model_as_printed}
            label="Label exported models as printed"
          />
          ...
          <div class="flex flex-col space-y-1.5">
            ...
            <Select.Root type="single" bind:value={configuration.startup_page}>
              <Select.Trigger class="w-full"
                >{configuration.startup_page in startPageOptions
                  ? startPageOptions[configuration.startup_page]
                  : "Default"}</Select.Trigger
              >
              <Select.Content>
                {#each Object.entries(startPageOptions) as [key, value] (key)}
    ...
    {#if sections.includes(SettingSection.UserInterface)}
      <Card>
        ...
        <CardContent class="flex flex-col gap-5 text-sm">
          ...
          />
          <CheckboxWithLabel
            bind:value={configuration.show_grouped_count_on_labels}
            label="Show grouped model count on labels"
          />
          <CheckboxWithLabel
            bind:value={configuration.show_date_on_list_view}
            label="Show date on list view"
          />
          <CheckboxWithLabel
            bind:value={configuration.only_show_single_image_in_groups}
            label="Only show first image of group"
          />
          <CheckboxWithLabel
            bind:value={configuration.show_multiselect_checkboxes}
            label="Show multiselect checkboxes"
          />
          <CheckboxWithLabel
            bind:value={configuration.use_worker_for_model_parsing}
            label="Use worker thread for model loading"
          />
          ...
          <div class="flex flex-col space-y-1.5">
            ...
            <Select.Root
              type="single"
              bind:value={configuration.group_split_view}
            >
              <Select.Trigger class="w-full"
                >{splitConversions[
                  configuration.group_split_view
                ]}</Select.Trigger
              >
              <Select.Content>
          ...
          <div class="flex flex-col gap-2 space-y-1.5">
            ...
            <div class="grid grid-cols-2 gap-2">
              <Select.Root
                type="single"
                bind:value={configuration.theme}
                onValueChange={(val) => setTheme(val)}
              >
                <Select.Trigger class="w-full"
                  >{getThemeName(configuration.theme)}</Select.Trigger
                >
                <Select.Content>
                  {#each getAvailableThemes() as theme (theme)}
                    <Select.Item value={theme}
                      >{getThemeName(theme)}</Select.Item
                    >
                  {/each}
                </Select.Content>
              </Select.Root>
            ...
            {#if configuration.theme === "custom"}
              <Textarea
                bind:value={configuration.custom_css}
                class="min-h-48"
                oninput={() => setTheme("custom")}
              />
            {/if}
          </div>
        </CardContent>
